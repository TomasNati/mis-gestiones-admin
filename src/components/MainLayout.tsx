...
export const MainLayout: React.FC = () => {
  const [anchorGestiones, setAnchorGestiones] = useState<null | HTMLElement>(null);
  const [anchorInversiones, setAnchorInversiones] = useState<null | HTMLElement>(null);
  const [view, setView] = useState<View>("categorias");

  const openGestiones = (e: React.MouseEvent<HTMLElement>) => setAnchorGestiones(e.currentTarget);
  const closeGestiones = () => setAnchorGestiones(null);
  const openInversiones = (e: React.MouseEvent<HTMLElement>) => setAnchorInversiones(e.currentTarget);
    ...
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <AppBar position="static" color="transparent" elevation={0} sx={{ borderBottom: 1, borderColor: "divider" }}>
        <Toolbar sx={{ display: "flex", gap: 2 }}>
          ...
          <Menu id="gestiones-menu" anchorEl={anchorGestiones} open={Boolean(anchorGestiones)} onClose={closeGestiones}>
            <MenuItem
              onClick={() => handleSelect("categorias")}
            >
              Categorias
            </MenuItem>
          </Menu>
          ...
          <Menu id="inversiones-menu" anchorEl={anchorInversiones} open={Boolean(anchorInversiones)} onClose={closeInversiones}>
            <MenuItem onClick={() => handleSelect("enumerados")}>Enumerados</MenuItem>
            <MenuItem onClick={() => handleSelect("instrumentos")}>Instrumentos</MenuItem>
          </Menu>
        </Toolbar>
      </AppBar>
